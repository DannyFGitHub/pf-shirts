  ...
  useFrame((state, delta) => {
    ...
    if (!useScrollBarPosition) {
      if (usePositionSlider) {
        ...
        shirtsGroupRef.current.position.set(position[0], position[1], position[2]);
        shirtsGroupRef.current.rotation.set(rotation[0], rotation[1], rotation[2]);
      }
    }

    if (useScrollBarPosition) {
      if (scroll.offset > 0 && scroll.offset < 0.33) {
        shirtsGroupRef.current.position.set(groupLocations[0].position[0], groupLocations[0].position[1], groupLocations[0].position[2]);
        shirtsGroupRef.current.rotation.set(groupLocations[0].rotation[0], groupLocations[0].rotation[1], groupLocations[0].rotation[2]);
      } else if (scroll.offset > 0.33 && scroll.offset < 0.66) {
        shirtsGroupRef.current.position.set(groupLocations[1].position[0], groupLocations[1].position[1], groupLocations[1].position[2]);
        shirtsGroupRef.current.rotation.set(groupLocations[1].rotation[0], groupLocations[1].rotation[1], groupLocations[1].rotation[2]);
      } else if (scroll.offset > 0.66 && scroll.offset < 1) {
        shirtsGroupRef.current.position.set(groupLocations[2].position[0], groupLocations[2].position[1], groupLocations[2].position[2]);
        shirtsGroupRef.current.rotation.set(groupLocations[2].rotation[0], groupLocations[2].rotation[1], groupLocations[2].rotation[2]);
      }
    }
  });
      ...
      <Center>
        <group ref={shirtsGroupRef} scale={[1, 1, 1]} rotation={[0, 0, 0]}>
          <primitive
            ...
            rotation={[0, -0.5, 0]}
          />
          <primitive ref={appleshirtRef} object={appleshirt.scene} position={[0, -0.3, 0]} children-0-castShadow rotation={[0, -0.5, 0]} />
          <primitive ref={cemshirtRef} object={cemshirt.scene} position={[0, -0.3, 2]} children-0-castShadow rotation={[0, -0.5, 0]} />
        </group>
